...
customerPins = ['123', '2575', '7275', '2312', '5049']
...
def saveCustomerData(): #At the end of every function/if statement
    global df
    dict = {'name': customerNames, 'Pin': customerPins, 'Balance': customerBalances}
    df = pd.DataFrame(dict)
    #df = df.drop(columns=["name","Pin","Balance"])
    df.to_csv('GFG2.csv', index = False)
    df.drop_duplicates()
    return
def loadCustomerData(): #At the beginning of every function/if statement
    global customerNames,customerPins,customerBalances
    #with open('GFG.csv', newline='') as f:
        #reader = csv.reader(f)
        #data = list(reader)
        #print(data[6])
    df = pd.read_csv("GFG2.csv", usecols=["name","Pin","Balance"], index_col=False)

    #Load new values into arrays
    for i in range(len(df)):
        x = df["name"][i]
        y = df["Pin"][i]
        z = df["Balance"][i]

        if (x == customerNames[i-1]) & (y == customerPins[i - 1]) & (z == customerBalances[i - 1]):
            print("Duplicates Found")
            df.drop_duplicates()


        else:
            customerNames.append(x)
            customerPins.append(str(y))
            customerBalances.append(z)
            df.drop_duplicates()
...
def removeDuplicatesFromNames():
    global customerNames
    customerNames = list(dict.fromkeys(customerNames))

def removeAllDuplicates():

    global customerNames,customerPins,customerBalances
    customerNames = list(dict.fromkeys(customerNames))
    customerPins = list(dict.fromkeys(customerPins))
    customerBalances = list(dict.fromkeys(customerBalances))
    return
